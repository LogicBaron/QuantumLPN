...
function lpn_result=lpn_feedback_reinforcement(n,p,E,M)
...
while k==1 || (k<2^(dataN) && lpn_result.sp_log(length(lpn_result.sp_log))<0.97)
    ...
    for epoch=1:E
        ...
        %measurement.
        %for each measurement output, calculate hamming distance between 
        ...
        %greedy algorithm.
        %compare the best measurement outcome of current stage, and guess
        %bit string we have. choose the one with the smaller hamming
        %distance compared to real output bit.
